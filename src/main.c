...
void lhc_args_free(void) {
  void *argtable[] = lhc_argtable;
  arg_freetable(argtable, sizeof(argtable) / sizeof(argtable[0]));
}

void lhc_args_parse(int argc, char **argv) {
...
int main(int argc, char **argv) {
  lhc_args_parse(argc, argv);
  
  // map args to cfg here 
  struct lhc_config cfg;
  memset(&cfg, 0, sizeof(cfg));

  cfg.verbose = verb->count > 0;

  int res = lhc_main(&cfg);
